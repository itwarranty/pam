...
if [ -f /run/mt-bastion/lab_mode ] || [ "${BASTION_GATEWAY_LAB_MODE:-0}" = "1" ]; then
...
if [ -f /run/mt-bastion/policy_v2_enabled ] || [ "${BASTION_GATEWAY_COMMAND_POLICY_V2_ENABLED:-0}" = "1" ]; then
  BASTION_GATEWAY_COMMAND_POLICY_V2_ENABLED=1
  if [ -f /run/mt-bastion/command_denylist ] || [ -f /etc/bastion/command_denylist ]; then
    export BASTION_DENYLIST=/run/mt-bastion/command_denylist
