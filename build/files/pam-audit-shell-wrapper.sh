...
LOG="/var/log/pam_sessions/audit_session_${USER}_$(date +%Y%m%d_%H%M%S).log"
META="${LOG}.meta"
...
logger -t gateway "audit session start user=${USER} log=${LOG} client=${SSH_CLIENT:-unknown}"
exec script -q -f -c "/usr/local/bin/pam-audit-shell.sh" "${LOG}"
