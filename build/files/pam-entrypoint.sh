...
OPERATORS_SRC="/etc/ssh-pam/operators"
TARGETS_SRC="/etc/ssh-pam/targets"
TARGETS_RUNTIME="/run/ssh-pam/targets-runtime"
...
if [ -f /etc/ssh-pam/command_denylist ]; then
  mkdir -p /run/ssh-pam
  cp -f /etc/ssh-pam/command_denylist /run/ssh-pam/command_denylist
  chmod 0644 /run/ssh-pam/command_denylist
fi

for _sess_dir in /run/ssh-pam/sessions; do
  mkdir -p "${_sess_dir}"
  chmod 1777 "${_sess_dir}" 2>/dev/null || true
done

mkdir -p /run/ssh-pam
[ "${PAM_GATEWAY_LAB_MODE:-0}" = "1" ] && echo 1 > /run/ssh-pam/lab_mode
[ "${PAM_GATEWAY_COMMAND_POLICY_V2_ENABLED:-0}" = "1" ] && echo 1 > /run/ssh-pam/policy_v2_enabled
[ "${PAM_SHELL_COMMAND_POLICY_V2_ENABLED:-0}" = "1" ] && echo 1 > /run/ssh-pam/shell_policy_v2_enabled

if [ -f /run/ssh-pam/lab_mode ] && [ -d "${TARGETS_RUNTIME}" ] && command -v ssh-keyscan >/dev/null 2>&1; then
...
if [ -d "${OPERATORS_SRC}" ]; then
  for src_dir in "${OPERATORS_SRC}"/*; do
    ...
    if [ -f "${home_dir}/permit_open" ] && [ -d "${TARGETS_RUNTIME}" ]; then
      rm -rf "${home_dir}/.itwarranty-pam"
      mkdir -p "${home_dir}/.itwarranty-pam/targets"
      if [ -f "${TARGETS_RUNTIME}/known_hosts" ]; then
        cp -f "${TARGETS_RUNTIME}/known_hosts" "${home_dir}/.itwarranty-pam/known_hosts"
      fi
      while read -r hp || [ -n "${hp}" ]; do
        ...
        for envf in "${TARGETS_RUNTIME}"/*/target.env; do
          ...
          if [ "${HOST}:${PORT}" = "${hp}" ]; then
            mkdir -p "${home_dir}/.itwarranty-pam/targets/${ID}"
            cp -f "${TARGETS_RUNTIME}/${ID}/target.env" "${home_dir}/.itwarranty-pam/targets/${ID}/target.env"
            if [ -f "${TARGETS_RUNTIME}/${ID}/identity" ]; then
              cp -f "${TARGETS_RUNTIME}/${ID}/identity" "${home_dir}/.itwarranty-pam/targets/${ID}/identity"
            fi
          fi
        done
    ...
    if [ -d "${home_dir}/.itwarranty-pam/targets" ]; then
      find "${home_dir}/.itwarranty-pam" -type f -exec chmod 600 {} \;
      chmod 700 "${home_dir}/.itwarranty-pam" "${home_dir}/.itwarranty-pam/targets" 2>/dev/null || true
    fi
  done
...
for logfile in /var/log/pam_sessions/*.log; do
  [ -f "${logfile}" ] || continue
  chattr +a "${logfile}" 2>/dev/null || true
done

touch /var/log/pam_sessions/gateway.syslog 2>/dev/null || true
chmod 0666 /var/log/pam_sessions/gateway.syslog 2>/dev/null || true
