#!/usr/bin/env bash
# Подключить уже запущенный ssh_pam к DMZ-сети lab (опционально).
set -euo pipefail
...
podman container exists "${CONTAINER}" || {
  printf 'Container %s not found — deploy gateway first (dev-up / site.yml)\n' "${CONTAINER}" >&2
  exit 1
}
...
printf 'Set allowed_sources to guest egress %s (not 10.1.13.0/24) if gateway is outside L2 guest VLAN.\n' "${GUEST_EGRESS_PUBLIC_IP}"
