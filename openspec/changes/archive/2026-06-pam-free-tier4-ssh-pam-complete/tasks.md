...
- [x] 1.1 Create `scripts/pam-session-search.sh` (JSONL filter, table + `--json`).
- [x] 1.2 Optional `--grep` over `.log` files with date window.
- [x] 1.3 Vars: `pam_session_search_enabled`; document `jq` package on Rocky 9.
- [x] 1.4 Install path: `/usr/local/bin/pam-session-search` via Ansible task (optional tag `session_search`).
...
- [x] 2.2 Create `build/files/pam-pty-inspector.py` + `pam-pty-inspector.sh` wrapper.
- [x] 2.3 Integrate in `pam-ssh-gateway-exec.sh` when `pam_gateway_command_policy_v2_enabled`.
- [x] 2.4 Integrate in `pam-shell-wrapper.sh` when `pam_shell_command_policy_v2_enabled`.
- [x] 2.5 Vars: `pam_gateway_deny_kill_session` in `group_vars/all.yml`.
...
- [x] 3.1 Create `scripts/pam-session-watch.sh` (tail -f registry log_path).
- [x] 3.2 JSONL `moderator_watch_start` events.
- [x] 3.3 Sudo/group doc: `pam_moderators` or audit SSH user.
- [x] 3.4 CSO Demo four-eyes block; Troubleshooting §5.7.
- [x] 3.5 Whitepaper row 30.

## 4. HashiCorp Vault (Phase D — v0.7.3)

- [x] 4.1 Extend `pam_targets` schema: `vault_secret_path`, `vault_secret_key`.
- [x] 4.2 Create `tasks/fetch_vault_target_keys.yml`; extend `provision_pam_targets.yml`.
...
- [x] 4.5 Vars: `pam_vault_*` in `all.yml.example`.
...
- [x] 5.4 Vars: `pam_oidc_*`, `pam_oidc_cert_policy_enabled` in example.
...
- [x] 6.3 Create `scripts/pam-ha-promote.sh`.
- [x] 6.4 Ansible: shared mount task for `audit_log_dir` when `pam_ha_enabled`.
...
- [x] 7.6 Extend `pam-compliance-verify.sh` (gateway manifest, jq optional).
...
Phase A: scripts/pam-session-search.sh
Phase B: pam-pty-inspector + gateway/shell integration, v1 fallback
Phase C: pam-session-watch.sh + JSONL moderator events
...
Phase F: HA runbook + ha.yml.example + pam-ha-promote.sh
