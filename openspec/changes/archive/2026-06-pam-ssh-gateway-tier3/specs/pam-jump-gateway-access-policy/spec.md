...
#### Scenario: pam_prod_require_gateway enabled
- **WHEN** `pam_prod_require_gateway` is `true`
- **AND** operator has `access: jump`
- **AND** operator `permit_open` intersects targets tagged `prod` (default tag check)
- **THEN** preflight SHALL fail unless operator has `pam_jump_approved: true` CSO waiver flag
...
#### Scenario: pam_jump_risk_acceptance_required
- **WHEN** `pam_jump_risk_acceptance_required` is `true`
- **AND** operator has `access: jump`
- **THEN** operator entry SHALL include `pam_jump_approved: true` or preflight SHALL fail
