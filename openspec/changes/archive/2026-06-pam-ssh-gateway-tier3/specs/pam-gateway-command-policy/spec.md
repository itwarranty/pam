...
When `pam_shell_command_policy_enabled` is `true`, gateway PTY SHALL apply the same denylist as shell sessions.
...
- **THEN** gateway SHALL read `/etc/ssh-pam/command_denylist` (existing Tier 2 mount)
