...
- [x] 1.1 Schema `pam_targets[]` in `group_vars/all.yml` + `all.yml.example` (id, host, port, account, identity_file, host_key_fingerprint, tags).
- [x] 1.2 Create `tasks/provision_pam_targets.yml` (keys, known_hosts, SELinux, permissions).
...
- [x] 2.1 Create `build/files/pam-ssh-gateway-wrapper.sh` (menu, session register, env).
- [x] 2.2 Create `build/files/pam-ssh-gateway.sh` (ssh client, script record, sidecars).
- [x] 2.3 Extend `build/Containerfile`: openssh-client, COPY wrappers, chmod.
- [x] 2.4 Mount `targets/` and `runtime/sessions/` in `deploy_ssh_pam.yml`.
...
- [x] 4.1 Session registry JSON in `{{ pam_home }}/runtime/sessions/`.
- [x] 4.2 Create `scripts/pam-session-ctl.sh` (list/kill; host wrapper with podman exec).
...
- [x] 5.1 Apply denylist to gateway PTY (extend `pam-command-policy-rc.sh` or gateway-local tap).
...
- [x] 6.2 `pam_gateway_lab_mode` for lab-only `accept-new`.
- [x] 6.3 auditd rule optional: read target identity files (`pam_target_key_read`).
...
- [x] 8.1 Vars: `pam_prod_require_gateway`, `pam_prod_target_tags`, `pam_jump_risk_acceptance_required`.
- [x] 8.2 Operator field `pam_jump_approved: true` waiver.
- [x] 8.3 Preflight rules per `gateway-jump-gateway-access-policy` spec.
...
- [x] 9.4 Extend `pam-compliance-verify.sh` for gateway lab smoke (optional).
