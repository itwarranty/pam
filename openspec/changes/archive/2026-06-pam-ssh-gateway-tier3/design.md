...
**Solution:** Add `access: gateway` — gateway **terminates** the operator SSH session and **originates** a new SSH session to the target (credential broking). All PTY I/O is captured on the gateway host under `audit_log_dir`.
...
| `shell` | Gateway-only / four-eyes / break-glass on the gateway | ✅ (gateway) | N/A |
| `audit` | Client read-only log review | ❌ | ❌ |

**CSO default policy (recommended):**

```yaml
pam_prod_require_gateway: false   # v0.6: opt-in; v0.7: recommend true for new prod
pam_jump_risk_acceptance_required: false  # when true, jump ops need pam_jump_approved: true
```
...
[Operator laptop]
    ...
    v
[Container sshd]
    |  Match User operator → ForceCommand /usr/local/bin/pam-ssh-gateway-wrapper.sh
    v
[gateway-wrapper.sh]
    |  1. Resolve permit_open ∩ pam_targets
    |  2. If single target → auto; else interactive menu (numbered list)
    |  3. Register session in /run/ssh-pam/sessions/<id>.json
    |  4. exec script → gateway.sh
    v
[gateway.sh]
    |  ssh -i /etc/ssh-pam/targets/<id>/id_ed25519 \
...
ProxyJump keeps gateway as **network relay**. Gateway mode requires **process-level PTY capture** on the gateway — only achievable when gateway runs `ssh` client to target.
...
Non-interactive CI: **out of scope v1** (future `PAM_GATEWAY_TARGET=host:port` env from Match SetEnv for break-glass automation only).
...
`authorized_keys`: **no** `restrict,port-forwarding` (operator needs PTY on the gateway for gateway shell).

### D2.5 Container packages

Add to `Containerfile`: ensure `openssh-client` present (Alpine: `openssh-client` or full `openssh` client binary). Verify `ssh` in PATH for `gateway` user inside container.
...
pam_targets:
  - id: prod-app-01                    # stable id for paths and logs
    host: 10.0.1.10
    port: 22
    account: pam_support                # Unix account ON TARGET
...
pam_operators:
  - name: engineer1
    access: gateway
    permit_open:
      - "10.0.1.10:22"                # MUST match a pam_targets entry
    incident_id: "INC-optional"
```

**Provisioning:**

1. Ansible copies/decrypts identity to `{{ pam_home }}/targets/<id>/id_ed25519` mode `0600`, owner `gateway`, SELinux `container_file_t`.
2. Mount RO: `.../targets:/etc/ssh-pam/targets:ro,Z`
3. Container entrypoint never copies target keys to operator `$HOME`.

**Preflight:**

- Every `gateway` operator `permit_open` entry MUST resolve to `pam_targets`.
- Target identity files MUST exist and be unreadable by non-`gateway`.
...
```text
/var/log/pam_sessions/gateway_<INCIDENT>_<OPERATOR>_<TARGETID>_<YYYYMMDD>_<HHMMSS>.log
```

Reuse `_sanitize_incident()` from `pam-shell-wrapper.sh`. `TARGETID` = `pam_targets[].id` (sanitized).
...
`gateway.sh` uses `StrictHostKeyChecking accept-new` **only in lab** (`pam_gateway_lab_mode: true`). Prod: **require** known_hosts file provisioned per target (`templates/ssh_known_hosts_targets.j2`).
...
Path on **host** (writable by container via mount): `/run/ssh-pam/sessions/` or `{{ pam_home }}/runtime/sessions/`
...
```json
{
  ...
  "log_path": "/var/log/pam_sessions/gateway_..."
}
```

### D5.2 CLI

`/usr/local/bin/pam-session-ctl` (host script, `podman exec` or sidecar):
...
Ansible tag: `session_kill` with `pam_session_kill_id` extra var.
...
- Source same `/etc/ssh-pam/command_denylist`
- Implement in `gateway.sh` via `ssh -tt` + `pam-command-policy-rc.sh` injected into remote shell **OR** local PTY tap (preferred: **local** — inspect bytes before write to ssh stdin)
...
When `pam_prod_require_gateway: true`:

- Operators with any `permit_open` matching `pam_prod_target_tags` (default `['prod']`) MUST have `access: gateway`.
- `access: jump` allowed only if `pam_jump_approved: true` on operator (CSO waiver flag).

When `pam_jump_risk_acceptance_required: true` and `access: jump`:

- Preflight requires `pam_jump_approved: true` and documents connect-audit-only limitation.
...
- `pam_targets` with lab key
- Operator `gateway-lab` with `access: gateway`

Acceptance: gateway session → log file exists → `sha256sum -c` → `pam-session-ctl list` shows session → kill works.
...
|:---|:---|:---|
| **A** | v0.6.0 | `pam_targets`, gateway wrappers, recording, sshd/keys |
...
| Risk | Mitigation |
|:---|:---|
| Target keys on the gateway = high value | RO mount, SELinux, 0600, auditd on key read, separate target SA with least privilege |
| Gateway compromise → all targets | Document; recommend per-target keys + no root; network ACL target←gateway only |
| `ssh -tt` latency / escape | Accept; document mosh not supported |
| Operator bypass via `jump` if both enabled | Policy Gate `pam_prod_require_gateway` |
