...
- [x] 1.1 Add to `group_vars/all.yml`:
  - `pam_require_fido_pubkey: false` (default)
  - `pam_mfa_mode: totp` (`totp` | `fido_totp` | `fido_only`)
  - `pam_fido_only_waiver: false`
  - `pam_fido_ecdsa_sk_allowed: false`
  - `pam_fido_waiver_operators: []`
  - `pam_fido_lab_enabled: false`
- [x] 1.2 Update `group_vars/prod.yml.example`: `pam_require_fido_pubkey: true`, `pam_mfa_mode: fido_totp`.
- [x] 1.3 Ensure `group_vars/dev/policy.yml` keeps `pam_require_fido_pubkey: false`.

## 2. Preflight (Phase B)

- [x] 2.1 Create `scripts/preflight-fido-key.py` — validate pubkey line or cert path is FIDO-sk backed; exit 0/1.
- [x] 2.2 Add `tasks/preflight_fido_operators.yml`; include from `preflight_cso.yml` when `pam_require_fido_pubkey`.
- [x] 2.3 Fail `pam_mfa_mode: fido_only` without `pam_fido_only_waiver: true`.
- [x] 2.4 Fail `fido_only` if `pam_require_fido_pubkey: false`.
- [x] 2.5 Warn (debug) for operators in `pam_fido_waiver_operators` when strict FIDO enabled.
...
- [x] 3.2 Preflight assert `pam_mfa_enforce: true` still required except documented fido_only path.
- [x] 3.3 Document that container image unchanged for fido_totp (no Containerfile change required).

## 4. Compliance verify (Phase D)

- [x] 4.1 Extend `scripts/pam-compliance-verify.sh` — check `fido_pubkey` when `PAM_REQUIRE_FIDO_PUBKEY=1` or vars file on host.
...
- [x] 5.3 Document interaction with `pam_cert_max_validity_hours` and JIT `valid_until`.
- [x] 5.4 OpenSpec cross-ref: `gateway-jit-access-windows`, `gateway-ssh-user-cert-operators`.

## 6. Lab (Phase F — optional)

- [x] 6.1 Create `group_vars/dev/fido_lab.yml` + merge in `operators_merge.yml` when enabled.
- [x] 6.2 `dev-up.sh`: if `PAM_FIDO_LAB=1`, generate `lab/keys/engineer-fido.lab` (sk) or print skip instructions.
...
- [x] 8.1 Merge `specs/pam-fido-anchor-mfa/spec.md` → `openspec/specs/pam-fido-anchor-mfa/spec.md`.
- [x] 8.2 Archive change to `openspec/changes/archive/2026-06-pam-fido-anchor-mfa/`.
...
- [ ] 9.1 Lab deploy with `pam_require_fido_pubkey: false` — regression: jump/shell/gateway still work.
- [ ] 9.2 Manual: FIDO-sk operator + TOTP login on Lima lab (if hardware available).
- [ ] 9.3 Preflight fail: non-sk pubkey when `pam_require_fido_pubkey: true`.
