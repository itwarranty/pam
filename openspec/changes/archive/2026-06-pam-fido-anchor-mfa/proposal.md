...
**Goal:** adopt **FIDO-Anchor + gateway TOTP** — phishing-resistant first factor on the **client workstation**, keep **offline TOTP** on the gateway, optional **short-lived SSH user certificates** aligned with JIT windows. **No** WebAuthn in container PAM, **no** `tsh`-like client, **no** IdP runtime dependency on the gateway (IdP path remains separate Tier 4 opt-in).
...
- Preflight: `pam_require_fido_pubkey: true` rejects non-`sk-ssh-*` pubkeys (waiver flag).
...
| `fido_only` | FIDO-sk only | CSO waiver only (`pam_fido_only_waiver: true`) |

- `pam_mfa_mode` in `group_vars/all.yml` (default `totp`; `prod.yml.example` → `fido_totp`).
- `pam_mfa_enforce: true` unchanged; `fido_only` relaxes PAM only with waiver.
...
- Preflight: when `pam_require_fido_pubkey` and certificate mode, cert MUST be backed by sk key (`ssh-keygen -Lf`).

### 4. Lab and migration

- `group_vars/dev/`: keep `pam_require_fido_pubkey: false` (existing `.lab` keys).
- Optional `group_vars/dev/fido_lab.yml` + `engineer-fido` lab operator with sk key fixture (generate in `dev-up.sh` if `PAM_FIDO_LAB=1`).
- Migration doc: phased rollout lab → pilot prod → `pam_require_fido_pubkey: true`.
...
| Capability | Spec |
|:---|:---|
| FIDO-Anchor MFA | `specs/pam-fido-anchor-mfa/spec.md` |

## Impact

- **Tasks:** `tasks/preflight_cso.yml`, `tasks/verify_compliance_cso.yml`
- **Scripts:** `scripts/pam-compliance-verify.sh`, `scripts/preflight-fido-key.py`, signing examples
...
- [ ] `fido_only` blocked unless `pam_fido_only_waiver: true`.
