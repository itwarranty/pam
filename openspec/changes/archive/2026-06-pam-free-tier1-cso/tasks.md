...
- [x] 1.2 Create `scripts/pam-compliance-verify.sh` (host-local, mirrors Ansible checks).
...
- [x] 2.1 Extend `build/files/pam-shell-wrapper.sh`: trap on EXIT writes `*.sha256` sidecar.
- [x] 2.2 Support optional env `PAM_INCIDENT_ID` (from operator `incident_id` via sshd `SetEnv` Match block — if feasible).
- [x] 2.3 Document verification in Troubleshooting Workflow §5.3.
- [x] 2.4 Optional: `tasks/archive_session_logs_worm.yml` when `pam_worm_archive_dir` set.
- [x] 2.5 Rebuild image via `trusted_download.sh`; update CSO demo ls + sha256 check.

## 3. Source IP restriction (Phase A)

- [x] 3.1 Extend `pam_operators` schema: `allowed_sources: []` in `all.yml.example`.
- [x] 3.2 Update `templates/authorized_keys.j2` with `from="..."` when list non-empty.
- [x] 3.3 Optional firewalld rich rules task when `pam_allowed_source_cidrs` set globally.
- [x] 3.4 Preflight assert when `pam_require_source_ip: true`.
- [x] 3.5 CSO demo: connect from wrong IP → fail.

## 4. SIEM syslog export (Phase A)

- [x] 4.1 Create `templates/rsyslog-pam-siem.conf.j2`.
- [x] 4.2 Create `tasks/configure_rsyslog_siem.yml` (install rsyslog if needed, drop-in, restart).
- [x] 4.3 Document CEF/field mapping appendix in spec `pam-siem-syslog-export`.
...
- [x] 6.3 Add `pam_allow_raw_pubkey_prod: false` default in `group_vars/all.yml`.
