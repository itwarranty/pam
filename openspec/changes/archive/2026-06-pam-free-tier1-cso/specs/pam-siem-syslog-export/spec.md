...
SSH PAM SHALL configure host-level rsyslog forwarding when enabled — without requiring Internet access from the gateway container.

#### Scenario: SIEM forwarding disabled by default
- **WHEN** `pam_siem_forward_enabled` is `false` (default)
...
- **WHEN** `pam_siem_forward_enabled` is `true`
- **AND** `pam_siem_server` is set
- **THEN** Ansible SHALL deploy rsyslog drop-in configuration to forward `local6.*` (auditd gateway keys) to `pam_siem_server:pam_siem_port`
...
### Requirement: Audit events for gateway SHALL use documented auditd keys

SIEM normalization SHALL rely on stable auditd key names already deployed.

#### Scenario: Required audit keys present
- **WHEN** gateway is deployed per current playbook
- **THEN** audit rules SHALL include keys at minimum:
  - `pam_session_logs` — writes to `audit_log_dir`
  - `pam_ssh_connect` — connect syscalls
...
|:---|:---|:---|
| auditd | `pam_session_logs` | Alert on unexpected delete/rename in session log dir |
| auditd | `pam_ssh_connect` | Correlate outbound connections from gateway UID |
...
| `deviceEventClassId` | audit key | `pam_session_logs` |
