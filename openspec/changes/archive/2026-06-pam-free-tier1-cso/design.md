...
- CA public: `pam_trusted_user_ca_file` → container `TrustedUserCAKeys`
- Operator: `operator.certificate` in `authorized_keys` (see existing templates)
- Private CA: never on the gateway; signing on admin workstation or client HSM
...
1. **Provisioning:** write `~/.pam-access-window` (or extend `.google_authenticator` comment) — informational.
2. **sshd:** `Match User` + `ForceCommand` wrapper that rejects login outside window (shell role) OR `AuthorizedKeysCommand` — **preferred: wrapper script** `gateway-access-window-check.sh` called before auth completes (PAM or ForceCommand pre-check).
...
- No live sshd time check; **purge-only**: timer removes expired operators from effective config by generating `pam_operators_active` filter in Ansible.
...
|:---|:---|:---|
| auditd (`pam_*` keys) | `local6` / `gateway-audit` | integrity, connect events |
| `/var/log/pam_sessions/` write detection | via auditd rule (existing) | session file creation |
| Optional: journald sshd on host | if forwarded | auth failures |

**Template:** `templates/rsyslog-pam-siem.conf.j2`

```properties
# Example — client replaces @@siem.example:514
local6.*    @@{{ pam_siem_server }}:{{ pam_siem_port | default(514) }}
```
...
pam_siem_server: ""
pam_siem_port: 514
pam_siem_protocol: tcp   # tcp|udp|relp — document client choice
```
...
2. **firewalld** (defense in depth) — optional global `pam_allowed_source_cidrs` rich rule on `pam_ssh_port`.

**Preflight (strict mode):**

```yaml
pam_require_source_ip: false   # prod CSO may set true
```
...
**Decision:** Extend `pam-shell-wrapper.sh`:

```sh
LOG="/var/log/pam_sessions/session_${USER}_$(date +%Y%m%d_%H%M%S).log"
META="/var/log/pam_sessions/session_${USER}_$(date +%Y%m%d_%H%M%S).log.meta"
# ... script session ...
...
```yaml
pam_worm_archive_dir: ""   # e.g. /mnt/worm/gateway/
```
...
1. **Shell script** `scripts/pam-compliance-verify.sh` — run on the gateway host (SSH or lima).
...
| 2 | SELinux | Enforcing |
| 3 | Container | `ssh_pam` running as `gateway` |
| 4 | Image label | `pam.mfa.strict=1` |
| 5 | Whitelist | `pam_permitted_targets` non-empty in deployed config |
| 6 | Orphan users | no `/home/*` in container outside `pam_operators` names |
| 7 | auditd | active |
| 8 | firewalld | active, port open |
| 9 | Session log dir | exists, mode 0750, owner gateway |
...
- [ ] Prod: mandate `pam_require_source_ip: true` by default?
