...
- Extend `pam_operators` schema with optional ISO8601 window fields.
...
- Ansible tasks: rsyslog forwarder config on the gateway **host** (not container).
...
- Enforce via `from=` in `authorized_keys.j2` and/or firewalld rich rules for `pam_ssh_port`.
...
- Optional Ansible task: copy closed logs to client WORM mount (`pam_worm_archive_dir`).
- Document CSO verification procedure (hash re-check before ticket attach).

### 6. Compliance verify (`verify` tag / script)

- `scripts/pam-compliance-verify.sh` and/or `tasks/verify_compliance_cso.yml`.
...
| Capability | Spec |
|:---|:---|
| SSH User CA (Free prod) | `specs/pam-ssh-user-ca-prod-free/spec.md` |
| JIT access windows | `specs/pam-jit-access-windows/spec.md` |
| SIEM syslog export | `specs/pam-siem-syslog-export/spec.md` |
| Source IP restriction | `specs/pam-source-ip-restriction/spec.md` |
| Tamper-evident session logs | `specs/pam-tamper-evident-session-logs/spec.md` |
| Compliance verify | `specs/pam-compliance-verify/spec.md` |

### Modified Capabilities

- `gateway-ssh-user-ca-trust` (from `ssh-user-ca-qa`) — prod defaults documented; no template change required if QA passes.
...
- **Templates:** `authorized_keys.j2`, `pam-shell-wrapper.sh`, new `rsyslog-pam.conf.j2`, `systemd/jit-ansible-run.timer.j2` (optional)
- **Scripts:** `scripts/pam-compliance-verify.sh`, extend `scripts/issue-operator-cert.sh` (optional stub)
- **Group vars:** `all.yml.example` — new operator fields; `pam_siem_*`, `pam_jit_*`, `pam_require_source_ip`
...
- [ ] `pam-compliance-verify.sh` passes on healthy lab deploy; fails on deliberate drift.
