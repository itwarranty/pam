...
- **WHEN** auditor runs `sha256sum -c session_*.log.sha256` on bastion host
...
- **WHEN** `bastion_worm_archive_dir` is empty
- **THEN** no archive task SHALL run

#### Scenario: WORM archive enabled
- **WHEN** `bastion_worm_archive_dir` is set to writable-only-by-automation mount
...
Hash sidecars complement — not replace — syslog/auditd pipeline in `bastion-siem-syslog-export`.
