...
- **AND** `bastion_allow_raw_pubkey_prod` is `false` (default)
...
- **THEN** signing procedure SHALL use `ssh-keygen -V` window ≤ 72 hours unless `bastion_cert_max_validity_hours` override is set with CSO documentation
...
- **THEN** CSO SHALL record passing results from `bastion-ssh-ca-qa-acceptance` scenarios
