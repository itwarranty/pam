...
- Complete QA from `ssh-user-ca-qa-mtglobal` and promote to **default prod recommendation**.
...
- `bastion-ssh-user-ca-trust` (from `ssh-user-ca-qa-mtglobal`) — prod defaults documented; no template change required if QA passes.
...
## Non-Goals (remain Paid / MT Access)
...
- **Blocked partially:** SSH User CA prod Free requires completion of `openspec/changes/ssh-user-ca-qa-mtglobal` (PKI access).
