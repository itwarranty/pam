...
| `ecdsa-sk` | Allowed when `bastion_fido_ecdsa_sk_allowed: true` |
...
| `bastion_mfa_mode` | `AuthenticationMethods` | PAM |
|:---|:---|:---|
| `totp` | `publickey,keyboard-interactive` | google-authenticator required |
| `fido_totp` | same | same (FIDO is first factor only) |
| `fido_only` | `publickey` | **No TOTP prompt** — only if `bastion_fido_only_waiver: true` AND `bastion_require_fido_pubkey: true` |

Implement `fido_only` via `templates/sshd_config.j2` conditional on mode + waiver. Preflight MUST fail `fido_only` without waiver.

**Default rollout:** `bastion_mfa_mode: fido_totp` + `bastion_require_fido_pubkey: true` in `prod.yml.example`; lab stays `totp` + `require_fido: false`.
...
Ansible `preflight_cso.yml` invokes via `ansible.builtin.command` with `changed_when: false` per operator when `bastion_require_fido_pubkey`.
