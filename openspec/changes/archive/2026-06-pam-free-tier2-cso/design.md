...
- Add **network-layer auth abuse** mitigation on the gateway SSH port.
...
```yaml
pam_break_glass_enabled: false   # prod: explicit CSO opt-in
pam_break_glass_max_hours: 4     # preflight cap on window length
...
pam_operators:
  - name: breakglass-oncall
    ...
    valid_until: "2026-06-15T06:00:00+03:00"   # REQUIRED; ≤ pam_break_glass_max_hours
```

**Enforcement:**

1. **Preflight:** reject `break_glass: true` when `pam_break_glass_enabled: false`; require `incident_id`, `valid_until`, window ≤ max hours.
2. **Provisioning:** write marker file `~/.pam-break-glass` (informational).
3. **Audit:** extra auditd key `pam_break_glass_session`; wrapper logs `BREAK_GLASS=1`.
...
- `build/files/pam-command-policy.sh` — reads denylist from `/etc/ssh-pam/command_denylist` (mounted RO from host).
- `pam-shell-wrapper.sh` sources policy; uses bash `DEBUG` trap or pre-exec hook to match command line against extended regex list.
- On match: log via `logger -t pam-deny`, optional auditd, print CSO message to user, do not execute.
...
**Decision:** **Host-level** controls (port published from Podman to host `pam_ssh_port`).
...
pam_ssh_rate_limit_method: firewalld   # firewalld | fail2ban
...
**Decision:** Extend existing `PAM_INCIDENT_ID` env (Tier 1) to **log basename**.
...
- Chroot not required v1; script validates paths under `/var/log/pam_sessions` only (realpath check).
