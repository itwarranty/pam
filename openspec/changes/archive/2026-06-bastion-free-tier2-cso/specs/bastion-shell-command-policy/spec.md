...
- **THEN** `bastion-shell-wrapper.sh` SHALL load denylist from container path `/etc/bastion/command_denylist`
...
- **WHEN** `bastion_shell_command_policy_enabled` is `true`
- **THEN** Ansible SHALL render denylist file on bastion host and mount read-only into container at `/etc/bastion/command_denylist`
