...
- **THEN** `sshd_config` Match block SHALL set `ForceCommand /usr/local/bin/bastion-audit-shell-wrapper.sh`
...
- **WHEN** audit operator runs `less`, `cat`, `ls`, `head`, `grep`, or `sha256sum` on path under `/var/log/bastion_sessions`
...
- **THEN** `bastion-audit-shell-wrapper.sh` SHALL record session via `script` with tamper-evident sidecars (same pattern as shell wrapper)
