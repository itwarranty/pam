...
- **WHEN** `bastion_break_glass_enabled` is `true`
- **AND** operator has `break_glass: true`, non-empty `incident_id`, and `valid_until`
- **AND** window duration is ≤ `bastion_break_glass_max_hours`
...
- **WHEN** operator has `break_glass: true`
- **AND** `valid_until - valid_from` (or `valid_until - now` if `valid_from` absent) exceeds `bastion_break_glass_max_hours`
- **THEN** preflight SHALL fail
...
- **WHEN** break-glass operator `valid_until` is before current bastion host time
- **AND** playbook runs (or `--tags jit_purge`)
- **THEN** operator SHALL be purged per `bastion-jit-access-windows` semantics
