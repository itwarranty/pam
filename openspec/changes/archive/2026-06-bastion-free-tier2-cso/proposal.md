...
- Global gate `bastion_break_glass_enabled: false` (prod opt-in).
...
- Configurable `bastion_shell_command_denylist` (default dangerous patterns).
- Enforced in `bastion-shell-wrapper.sh` for `access: shell` only.
- Blocked commands logged to auditd/syslog; session continues.

### 3. SSH brute-force / rate protection

- Host-level protection for `bastion_ssh_port` (firewalld rate limit and/or fail2ban).
...
- New operator role: view session logs under `/var/log/bastion_sessions` only.
- No ProxyJump, no shell on targets, no write to log directory.
- ForceCommand → `bastion-audit-shell-wrapper.sh`.
...
| Capability | Spec |
|:---|:---|
| Break-glass emergency access | `specs/bastion-break-glass-access/spec.md` |
| Shell command denylist | `specs/bastion-shell-command-policy/spec.md` |
| SSH brute-force protection | `specs/bastion-ssh-brute-force-protection/spec.md` |
| Incident-correlated log naming | `specs/bastion-incident-log-naming/spec.md` |
| Read-only audit role | `specs/bastion-audit-readonly-role/spec.md` |

### Modified Capabilities

- `bastion-jit-access-windows` — break-glass operators MUST use `valid_until`; purge semantics unchanged.
- `bastion-tamper-evident-session-logs` — filename pattern when `incident_id` present.
- `bastion-compliance-verify` — optional checks for fail2ban/firewalld rate limit and break-glass gate.

## Impact

- **Tasks:** `tasks/configure_ssh_brute_force.yml`, extend `preflight_cso.yml`, `provision_operator_item.yml`
- **Build:** `build/files/bastion-shell-wrapper.sh`, new `bastion-audit-shell-wrapper.sh`, optional `bastion-command-policy.sh`
...
- [ ] `bastion-compliance-verify.sh` passes on lab deploy with Tier 2 features enabled.
