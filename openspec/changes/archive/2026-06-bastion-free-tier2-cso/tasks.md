...
- [x] 1.1 Update `build/files/bastion-shell-wrapper.sh`: sanitized `incident_id` in log basename.
...
- [x] 2.1 Add `bastion_shell_command_policy_enabled`, `bastion_shell_command_denylist` to `group_vars/all.yml` + example.
- [x] 2.2 Create `build/files/bastion-command-policy.sh` + integrate in shell wrapper (DEBUG trap or equivalent).
- [x] 2.3 Ansible: render `templates/command_denylist.j2` → host mount RO in container (`/etc/bastion/command_denylist`).
...
- [x] 3.1 Create `build/files/bastion-audit-shell-wrapper.sh` (path guard + allowed commands).
