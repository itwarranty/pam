...
- **THEN** `authorized_keys` SHALL prefix the certificate with `restrict,port-forwarding` and `permitopen` entries from `permit_open` or global `bastion_permitted_targets`
