...
### 1. Session search CLI (`bastion-session-search`)
...
- No proprietary database — files on bastion host only.

### 2. Gateway command policy v2 (local PTY inspector)

- Replace remote `bash --rcfile` injection with **bastion-side PTY tap** (pipe/expect-class) for gateway sessions.
- Denylist + optional allowlist; deny logs + optional auto-kill session.
- Same policy file `/etc/bastion/command_denylist`.

### 3. Live session moderation

- `bastion-session-watch <session-id>` — read-only live tail of active gateway PTY log for client moderator (`access: audit` or dedicated `access: moderator` — **decision: extend audit role** with watch permission).
- Optional notify via syslog when moderator attaches.

### 4. External secrets (HashiCorp Vault)

- `bastion_targets[].vault_path` fetches SSH private key at provision time (Ansible `community.hashi_vault`).
...
- Air Gap: offline signing helper + documented token export; no bastion→IdP runtime dependency required in prod.

### 6. HA active-passive (two bastion hosts)
...
| Capability | Spec |
|:---|:---|
| Session search CLI | `specs/bastion-session-search/spec.md` |
| Gateway command policy v2 | `specs/bastion-gateway-command-policy-v2/spec.md` |
| Live session moderation | `specs/bastion-live-session-moderation/spec.md` |
| External secrets (Vault) | `specs/bastion-external-secrets-vault/spec.md` |
| OIDC/SAML SSH user certs | `specs/bastion-oidc-saml-ssh-certificates/spec.md` |
| HA active-passive | `specs/bastion-ha-active-passive/spec.md` |
| v1.0 GA release | `specs/bastion-v1-ga-release/spec.md` |

## Impact

- **Build:** `bastion-pty-inspector.sh`, `bastion-session-search.sh`, `bastion-session-watch.sh`
- **Tasks:** `fetch_vault_target_keys.yml`, `configure_ha_standby.yml`, extend `provision_bastion_targets.yml`
...
- [ ] `bastion-session-search --operator X --since 7d` returns gateway sessions in &lt;5s on 10k JSONL lines.
- [ ] Gateway denylist blocks `rm -rf /` without remote rc injection (v2 inspector).
- [ ] Moderator with `access: audit` can `bastion-session-watch` active session.
