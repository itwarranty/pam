...
#### Scenario: firewalld rich rule for gateway port
- **WHEN** `pam_allowed_source_cidrs` is non-empty
- **THEN** Ansible SHALL configure firewalld to allow `pam_ssh_port` only from those CIDRs
...
- **WHEN** `pam_require_source_ip` is `true`
- **AND** any operator in `pam_operators` lacks non-empty `allowed_sources`
...
- **THEN** `pam_require_source_ip` SHOULD default to `false` unless explicitly overridden
