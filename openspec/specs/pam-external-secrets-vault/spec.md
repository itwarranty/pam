...
SSH PAM SHALL support external secret store for `pam_targets` credentials without runtime Vault dependency in container.

#### Scenario: Vault path configured
- **WHEN** target entry has `vault_secret_path` and `pam_vault_enabled: true`
- **THEN** Ansible SHALL fetch private key from Vault KV v2 on controller during provision
- **THEN** key SHALL be written to `{{ pam_home }}/targets/<id>/identity` with mode 0600
...
- **WHEN** `pam_vault_enabled` is `false`
...
#### Scenario: No Vault connectivity from gateway host
- **WHEN** Ansible controller can reach Vault but gateway host cannot
