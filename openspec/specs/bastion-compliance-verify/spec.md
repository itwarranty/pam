...
- **THEN** `scripts/bastion-compliance-verify.sh` SHALL be present and executable

#### Scenario: Script exit code indicates pass/fail
- **WHEN** all checks pass on a correctly deployed Rocky 9 bastion
...
- **WHEN** verify runs on bastion host
...
- **THEN** it SHALL assert:
  - `bastion_permitted_targets` is non-empty (from host-side deployed sshd_config or ansible vars file if present)
...
- **WHEN** `bastion_command_policy_v2_required` is `true`
- **AND** `bastion_shell_command_policy_enabled` is `true`
- **THEN** verify SHALL assert container has `/run/bastion/policy_v2_enabled` and `/run/bastion/shell_policy_v2_enabled`
- **THEN** verify SHALL assert `bastion-pty-inspector.py` exists in image
