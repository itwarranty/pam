...
- **THEN** bastion SHALL NOT implement biometric APIs — verification is client-side only
...
- **WHEN** `bastion_require_fido_pubkey` is unset or `false`
- **THEN** deploy SHALL accept legacy non-sk pubkeys (subject to existing `bastion_allow_raw_pubkey_prod` / certificate rules)

#### Scenario: Prod strict mode
- **WHEN** `bastion_require_fido_pubkey: true`
- **AND** operator uses `pubkey` (not empty)
- **THEN** preflight SHALL fail if pubkey does not start with `sk-ssh-ed25519@openssh.com` (or allowed ecdsa-sk when `bastion_fido_ecdsa_sk_allowed: true`)
...
- **WHEN** `bastion_require_fido_pubkey: false` with explicit `bastion_fido_waiver_operators: [name]` documented
- **THEN** preflight MAY allow listed operators to use non-sk keys (migration only; warn in debug)

### Requirement: TOTP on bastion SHALL remain mandatory unless explicit fido_only waiver

FIDO strengthens the **first** factor; offline TOTP remains the default **second** factor.

#### Scenario: Default MFA mode fido_totp
- **WHEN** `bastion_mfa_mode: fido_totp` (recommended prod)
...
- **WHEN** `bastion_mfa_mode: totp`
- **THEN** behavior SHALL match v1.0.0 (any allowed pubkey/cert + TOTP)

#### Scenario: fido_only mode gated
- **WHEN** `bastion_mfa_mode: fido_only`
- **AND** `bastion_fido_only_waiver` is not `true`
- **THEN** preflight SHALL fail
- **WHEN** `bastion_fido_only_waiver: true` AND `bastion_require_fido_pubkey: true`
...
- **WHEN** `bastion_require_fido_pubkey: true` on deployed host
- **THEN** `scripts/bastion-compliance-verify.sh` SHALL verify all configured operators use FIDO-sk pubkeys or FIDO-backed certificates
...
- **THEN** script SHALL produce certificate with `-V` matching JIT window (≤ `bastion_cert_max_validity_hours` unless CSO extends)
...
- **THEN** Policy Gate rows SHALL document `bastion_require_fido_pubkey` and `bastion_mfa_mode`
