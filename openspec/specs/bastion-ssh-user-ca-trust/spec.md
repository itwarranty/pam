...
When `bastion_trusted_user_ca_file` is set, MT: Bastion SHALL install the org User CA public key on the bastion host and expose it to container sshd as `/etc/ssh/trusted_user_ca.pub` with `TrustedUserCAKeys` enabled.
...
MT: Bastion deployment artifacts SHALL NOT include or copy the User CA private key to bastion host, container, or git repository.
